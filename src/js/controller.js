...
const controlRecipes = async function () {
  try {
    ...
    if (!id) return;
    recipeView.renderSpinner(); // calls method from recipeView
    ...
    // 1) Loading recipe
    await model.loadRecipe(id); // sends the id to the model method

    // 2) Rendering recipe
    recipeView.render(model.state.recipe); // sends the data from the state to the recipeView's render method
  } catch (err) {
